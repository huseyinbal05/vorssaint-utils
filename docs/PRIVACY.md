...
5. **Video Downloader, only after you enter a valid link.** Inspection and download contact the pasted site from your Mac through the locally installed `yt-dlp`. Inspection starts only after the text is a valid public HTTP or HTTPS URL. When inspection supplies a remote preview-image URL, the app loads that image directly from its host to show the thumbnail. The media URL is not saved, logged or placed in command-line arguments; it is sent to `yt-dlp` over standard input. Browser cookies, account sessions, `.netrc`, personal yt-dlp configuration and user plugins are not used. The feature accepts one public, non-live media item at a time and rejects playlists, channels, private or restricted items and DRM media. Installing missing `yt-dlp` or `ffmpeg` tools may contact Homebrew, GitHub and package hosts; when Homebrew itself is absent, the official installer opens in Terminal and may ask for confirmation or a password.
